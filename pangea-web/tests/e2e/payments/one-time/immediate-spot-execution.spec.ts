...
currencyPairs.forEach((currencyPair: CurrencyPairsType) => {
  test.describe(`Immediate Spot Execution Payment Workflow buying ${currencyPair.buy} selling ${currencyPair.sell} with ${currencyPair.liquidity} liquidity`, () => {
    ...
    test(`should be able to book transaction that executes immediately in normal flow buying ${currencyPair.buy} selling ${currencyPair.sell}`, async ({
      page,
    }) => {
      // Setting Variables for the test specs
      const formValues: bookTransactionTestFormValuesType = {
        description: `${currencyPair.buy} ${currencyPair.sell} Spot Execution Flow with ${currencyPair.liquidity} liquidity e2e test`,
      ...
      await setSpotExecutionDate(page);
    ...
    test(`should be able to book transaction that executes immediately after editing amount, description and purpose of payment along with refresh rate for ${currencyPair.buy} selling ${currencyPair.sell}`, async ({
      page,
    }) => {
      // Setting Variables for the test specs
      const initialFormValues: bookTransactionTestFormValuesType = {
        description: `${currencyPair.buy} ${currencyPair.sell} Spot Execution Workflow with ${currencyPair.liquidity} liquidity e2e test`,
        sellCurrency: currencyPair.sell,
        buyCurrency: currencyPair.buy,
        sellAmount: '10.00',
        purposeOfPayment: 'Intercompany payment',
      ...
      const finalFormValues: bookTransactionTestFormValuesType = {
        description: `Update ${currencyPair.buy} ${currencyPair.sell} Spot Execution Workflow with ${currencyPair.liquidity} liquidity  e2e test`,
        sellCurrency: currencyPair.sell,
        buyCurrency: currencyPair.buy,
        sellAmount: '15.00',
