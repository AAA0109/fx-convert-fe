...
const currencyPairs = getCurrencyPairs(['good', 'acceptable'], 'manual');
let executionTimingPromise: Promise<Response>;

currencyPairs.forEach((currencyPair: CurrencyPairsType) => {
  test.describe(`Strategic Forward Execution Payment Workflow for no electronic payments buying ${currencyPair.buy} selling ${currencyPair.sell} with ${currencyPair.liquidity} liquidity `, () => {
