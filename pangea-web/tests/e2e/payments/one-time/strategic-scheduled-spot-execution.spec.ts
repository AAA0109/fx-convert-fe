...
const currencyPairs = getCurrencyPairs(['good']);
let executionTimingPromise: Promise<Response>;

currencyPairs.forEach((currencyPair: CurrencyPairsType) => {
  test.describe(`Scheduled Spot Strategic Execution Payment Workflow buying ${currencyPair.buy} selling ${currencyPair.sell} with ${currencyPair.liquidity} liquidity`, () => {
    ...
    test(`should be able to book transaction with strategic execution in normal flow buying ${currencyPair.buy} selling ${currencyPair.sell}`, async ({
      ...
      pangeaPaymentResponse = await setDeliveryMethod(
        page,
        DeliveryMethods.ScheduledStrategic,
      ...
      await validateTransactionConfirmationPage(page, 'scheduled');
